...
export async function extractFramesFromVideo(
  ...
  const ffmpegParams = [
    ...
    'image2pipe',
    '-vf',
    `fps=${Constants.VIDEO_SAMPLE_RATE}`,
    'pipe:1',
  ];
  ...
  return new Promise(async (resolve, reject) => {
    ...
    ffmpeg.on('exit', (code, signal) => {
      if (code !== 0) {
        ...
        reject(
          new Error(`FFmpeg exited with code ${code} and signal ${signal}`),
        );
      } else {
        resolve(frames);
      }
...
export function interpretPrediction(
  ...
  for (const prediction of predictions) {
    if (
      Constants.VIDEO_CATEGORIES_TO_FLAG.includes(prediction.className) &&
      prediction.probability >
        Constants.VIDEO_CATEGORIES_PREDICTION_MIN_THRESHOLD
    ) {
      isExplicit = true;

      if (
        prediction.probability <
        Constants.VIDEO_CATEGORIES_PREDICTION_MAX_THRESHOLD
      ) {
        review = true;
      }
    }
