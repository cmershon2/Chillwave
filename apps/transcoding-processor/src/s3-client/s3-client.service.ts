  ...
  async getVideoStream(bucket: string, key: string): Promise<Readable> {
    ...
    try {
      ...
      if (response.Body instanceof Readable) {
        this.logger.log(
          `Successfully retrieved stream for ${key} from ${bucket}`,
        );
  ...
  async uploadFile(data: S3FileUpload): Promise<string> {
    ...
    try {
      ...
      upload.on('httpUploadProgress', (progress) => {
        const completionCalculation = Math.round(
          (progress.loaded / progress.total) * 100,
        );
        console.log(
          `Upload Progress: ${completionCalculation}% - ${progress.loaded}/${progress.total}`,
        );
      });

      const uploadResult = await upload.done();
      this.logger.log(
        `Successfully uploaded ${params.Key} to ${data.bucketName}`,
      );
