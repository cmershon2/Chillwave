...
export class ContentFilteringProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>) {
    try {
      ...
      const filterResult =
        await this.contentFilterService.detectExplicitVideoContent(
          job.data.key,
        );
      ...
      await job.moveToFailed(
        new Error(`Error processing video upload: ${error}`),
        job.token,
      );
      throw error;
    }
  ...
  async onFailed(job: Job, error: any) {
    this.logger.error(
      `Job ${job.id} of type ${job.name} has failed. Error: ${error}`,
    );
  }
